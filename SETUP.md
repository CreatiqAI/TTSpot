...
- **Toolbar layout (0.3.23).** `_Action` (left: TT now / Plan / Suggest; black while my TT is live, white for a friend's) + `_Status` (icon or avatar stack, one line of text, round caret button with the moments count as a badge). GoogleMap `onTap` closes the sheet when it is open.
