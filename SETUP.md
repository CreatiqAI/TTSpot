...
- **Toolbar layout (0.3.23).** 60 high, 44 buttons like the tab bar. `_Action` (left: always the TT button; black while my TT is live, white for a friend's) + `_Status` (icon or avatar stack, one line of text, round caret button with the moments count as a badge). GoogleMap `onTap` closes the sheet when it is open.
